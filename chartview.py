...
class ChartView(QChartView):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        if self.is_clicking:
            ...
            delta_y = y_max - y_min
            index = int((x_event - 85) / (470 / (x_max - x_min)) + x_min)
            y = (y_event - 35) / (240 / (y_max - y_min))
            y = y_max - y
            if 0 <= index < len(self.ui.original_data_1) and len(self.ui.series_1.pointsVector()) > index:
                y1 = self.ui.original_data_1[index].y()
                y2 = self.ui.original_data_2[index].y()
                # print(x_event, y_event, index, y)

                if abs(y1 - y) < delta_y * 0.1 and 85 < x_event < 856:
